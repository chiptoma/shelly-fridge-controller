...
import type { Logger, LogLevel, InitMessage } from '@logging';
...
export function initialize(): Controller | null {
  const t = now();
  ...
  const isInfo = CONFIG.GLOBAL_LOG_LEVEL <= CONFIG.LOG_LEVELS.INFO;

  if (isInfo) {
    logInitSummary(logger, state);
  }

  return { state, logger, isDebug };
}
